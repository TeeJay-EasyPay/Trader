...
function ExperimentDetail({ request, id, onBack }) {
  ...
  return <View style={s.card}><Button label="Back" onPress={onBack} /><Text style={s.title}>Experiment report</Text>
    ...
    {data && <><Text style={s.text}>Hypothesis: {data.spec.hypothesis}</Text><Text style={s.small}>{human(data.display_status || data.status)} · {human(data.spec.broker)} · {data.spec.currency || 'USD'} · SIMULATED</Text>
      ...
      <Text style={s.small}>{data.spec.portfolio_basis === 'independent-shadow-capacity-v1' ? 'Independent virtual capital: only the real-account capital-allocation block can be replaced. Each arm keeps a 25% virtual exposure cap, cash, position, stop and data safeguards. Older results are not mixed into this version.' : 'Recorded account eligibility: real-account capacity blocks also block this legacy comparison.'}</Text>
      ...
      <Text style={s.text}>Both skipped: {data.report.both_skipped ?? 'awaiting refreshed report'} · Informative completed comparisons: {data.report.informative_completed ?? 'awaiting refreshed report'} · Different decisions: {data.report.decision_differences ?? 'not reported'}</Text>
      {Object.entries(data.report.blocker_counts || {}).map(([reason, count]) => <Text key={reason} style={s.small}>{human(reason)}: {count} paired skips</Text>)}
      <Text style={s.small}>Standalone shadow outcomes elsewhere in Learning are a separate dataset, not additional paired comparisons. Simulated costs are estimates; skips alone do not establish learning or profit.</Text>
