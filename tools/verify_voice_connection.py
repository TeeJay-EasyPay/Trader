...
Usage records are isolated in a temporary SQLite database. One short response.
"""
import argparse
import asyncio
import json
    ...
    with tempfile.TemporaryDirectory(prefix='trader-voice-smoke-') as directory:
        ...
        try:
            with patch('ai_trader.self_assessment.input_inventory',return_value={'synthetic_test':True}):
                result=await asyncio.to_thread(v.start,service,dict(mode='trader',confirmed=True,sdp=pc.localDescription.sdp))
            ...
            if result:
                ...
                for _ in range(20):
                    if v.budget(db)['session_id'] is None:
                        break
                    await asyncio.sleep(.5)
                print(json.dumps({'final_budget':v.budget(db)}))
            await pc.close()
